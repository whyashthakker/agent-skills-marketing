...
## Case Study: Next.js Migration SEO Disaster (ThemeFischer)

**Source:** [r/nextjs – "We lost 85% of our search traffic because of one mistake"](https://www.reddit.com/r/nextjs/comments/1rv24j2/we_lost_85_of_our_search_traffic_because_of_one/)

### What Happened

During a Next.js App Router migration, SEO-critical components (meta tags, title, description, OG tags) ended up **client-side rendered**. Googlebot received HTML with no title, no description, and mostly blank content → **85% search traffic loss**.

### Root Cause

1. **Custom SEO component** instead of Next.js metadata API – meta tags were in a component that got moved into a client component for interactive state.
2. **BAILOUT_TO_CLIENT_SIDE_RENDERING** – misuse of APIs like `useSearchParams()` in a static page forced the whole page to CSR.
3. **No crawler verification** – dev checked locally (hydration works in browser) but never verified what Googlebot sees.

### Prevention Checklist

| Check | How |
|-------|-----|
| Use metadata API | `metadata` / `generateMetadata` in server components only – never custom SEO components in App Router |
| Never meta in client | Meta tags must be in server components; `"use client"` still SSR by default, but bailouts can force CSR |
| Test without JS | Disable JavaScript and load page – if content is blank, crawlers see the same |
| View source | `View Source` (Ctrl+U) shows server HTML; verify title, meta, and main content are present |
| Crawl before deploy | Crawl staging the same way Googlebot would; diff against production baseline before DNS switch |
| Lighthouse on prod | Run Lighthouse on production pages post-deploy |
| Staging comparison | Crawl old vs new env; any unexpected change in titles, canonicals, or indexable content is a red flag |

### Key Takeaway

> **"use client"** does not mean "client-only" – Next.js still SSR by default. The real risk is **bailout** (e.g. `useSearchParams()` on static pages) or putting metadata in components that get excluded from SSR. Use the [Next.js metadata API](https://nextjs.org/docs/app/getting-started/metadata-and-og-images#static-metadata) and keep SEO-critical output in server components.
